 ...
 *        computing between worker threads
 */
...
class FftProcessor
{
  public:
     ...
     *
     * @param audioFile A vector of float with the audio samples inside.
     * @return std::shared_ptr<std::vector<float>>  A vector of resulting fourier transform.
     */
    std::shared_ptr<std::vector<float>> performFft(std::shared_ptr<std::vector<float>> audioFile);
  ...
  private:
     ...
     */
    void processJob(std::shared_ptr<FftProcessorJob> jobRef, mufft_plan_1d *plan, float *in, cfloat *out);
    ...
    std::queue<std::shared_ptr<FftProcessorJob>> todoJobQueue; /**< queue of jobs to be picked by workers */
    std::queue<std::shared_ptr<FftProcessorJob>>
    ...
    std::queue<std::shared_ptr<std::vector<std::shared_ptr<FftProcessorJob>>>>
