...
#include "StationPlugin/PluginEditor.h"
      ...
      audioTransportGrpcClient(DEFAULT_SERVER_PORT), audioInfoForwarder(audioTransportGrpcClient)
{
    ...
    if (const char *logLevel = std::getenv("KHOLORS_LOG_LEVEL"))
    {
        if (std::strcmp(logLevel, "DEBUG") == 0)
        {
            ...
            spdlog::debug("KHOLORS_LOG_LEVEL was set so the Kholors Station plugin will show debug info");
        }
    }
}
...
{
    return new AudioPluginAudioProcessorEditor(*this, taskManager);
}

void AudioPluginAudioProcessor::getStateInformation(juce::MemoryBlock &)
{
    // TODO: pull the settings from the UI sliders and zoom level
}

void AudioPluginAudioProcessor::setStateInformation(const void *, int)
{
    // TODO: set the settings from the UI sliders and zoom level
}
