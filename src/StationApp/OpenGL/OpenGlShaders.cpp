...
std::string volumesVertexShader =
    R"(
#version 330 core
...
std::string volumesFragmentShader =
    R"(
#version 330 core
out vec4 FragColor;
...
uniform sampler2D volumeBarsTexture;
uniform int convolutionId;

void main()
{
    vec4 textureColor = texture(volumeBarsTexture, TexCoord);
    FragColor = vec4(textureColor.x, textureColor.y, textureColor.z, textureColor.a);
}
)";
